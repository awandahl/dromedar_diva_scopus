...
### Fetching CSV file from DiVA via httr/curl:
...
GET(diva_url, write_disk("export.csv", overwrite=TRUE))

## Import from DiVA: CSV with fields DOI, Scopus-ID, ISI, PMID.
...
if(file.exists('dois_missing_eid')) {
  ...
  result_eid_for_dois <- extractXML(theXML_eid)
}
...
if(file.exists('dois_missing_pmid')) {
  ...
  result_pmid_for_dois <- extractXML(theXML_pmid_doi)
}
...
## Merge output file from Scopus search with the original file from DiVA. We want to have the DiVA PIDs along with the new data
...
if(exists('result_pmid_for_dois')) {
  ...
  split_csv(new_pmids) ## using function csv_split.R instead of above
